var RadarChart = {
  ...
  chart: function() {
		...
		selection.each(function(data) {
			...
			if (cfg.levelTick){
				levelLine
					...
					.attr('transform', function(levelFactor) {
						return 'translate(' + (cfg.width/2-levelFactor) + ', ' + (cfg.height/2-levelFactor) + ')';
					});
			}
			else{
				levelLine
			...
			if(cfg.axisLine || cfg.axisText) {
				...
				if(cfg.axisLine) {
					axis.select('line')
						...
						.attr('x2', function(d, i) { return getHorizontalPosition(i, cfg.width / 2, cfg.factor); })
						.attr('y2', function(d, i) { return getVerticalPosition(i, cfg.height / 2, cfg.factor); });
				}
				...
				if(cfg.axisText) {
					axis.select('text')
						...
						.attr('x', function(d, i){ return d.xOffset+ getHorizontalPosition(i, cfg.width/2, cfg.factorLegend); })
						.attr('y', function(d, i){ return d.yOffset+ getVerticalPosition(i, cfg.height/2, cfg.factorLegend); });
				}
			}
			...
			data.forEach(function(d){
				d.axes.forEach(function(axis, i) {
					axis.x = getHorizontalPosition(i, cfg.width/2, (parseFloat(Math.max(axis.value, 0))/maxValue)*cfg.factor);
					axis.y = getVerticalPosition(i, cfg.height/2, (parseFloat(Math.max(axis.value, 0))/maxValue)*cfg.factor);
				});
			});
	...
	draw: function(id, d, options) {
		...
		var place = (typeof id == 'string')? d3.select(id) : id;
