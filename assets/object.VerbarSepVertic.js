function SVGVerbarSepVerticDrawer (input){
...
// Prepare values
	...
	for (var date in dataset.charts[0].data){
		for(var chart in dataset.charts){
			datatemp.push({'name':dataset.charts[chart].chart,'value':dataset.charts[chart].data[date].real*flip})
		}
	}
	...
	function _add_graph(main_svg,cord_x,cord_y,height,width,data,index,max,min){
		...
		if (min>0){
			min=0
		}else sub_height=sub_height-add_margin*1
			...
			svg.selectAll("_")
				...
				.enter().append("text")
					...
					.attr("y",function(d) {
						...
						}else{ return value-add_margin*.3;}
					})
					.text(function(d, J) {
